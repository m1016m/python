...
#我們要的油價表在 tag <span id=Showed>
#從bsObj(一個tree Obj) 中,以findAll() method
#find all <span> tag ,id=Showed
#findAll(),回傳的是一個 list (list of tag)
data=bsObj.findAll('span',{'id':'Showtd'}) 

#從data (list)中第一個元素(<span> tag)中
#以findAll() method 找出所有 <tr> tag
#回傳的仍是一個 list (list of <tr> tag)
rows=data[0].findAlll('tr')
